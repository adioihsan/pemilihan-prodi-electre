...
<x-layout bodyClass="g-sidenav-show bg-gray-200"  :alert="$alert">
    ...
    <div class="main-content position-relative bg-gray-100 max-height-vh-100 h-100">
        <!-- Navbar -->
        <x-navbars.navs.auth titlePage='Tambah Kriteria' :page_ref="$page_ref" ></x-navbars.navs.auth>
        <!-- End Navbar -->
        <div class="container-fluid px-2 px-md-4">
            ...
            <div class="card card-body mx-3 mx-md-4 mt-6">
                <h6>Data Kriteria</h6>
                <form method='POST' action='{{ route('create-kriteria') }}' enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            ...
                            <div class="mb-3 col-md-4">
                                ...
                                <select class="form-control border border-2 p-2" name="tipe_bobot" id="tipe-bobot" >
                                    @foreach($list_tipe_bobot as $bobot)
                                        <option value="{{$bobot->tipe}}">{{$bobot->tipe}}</option>
                                    @endforeach
                                </select>
                                @error('bobot')
                                <p class='text-danger inputerror'>{{ $message }} </p>
                        ...
                        <div class="row">
                            <div class="mb-3 col-12">
                                ...
                                <textarea class="form-control border border-2 p-2" name="pertanyaan" rows="5" placeholder="Tulis pertanyaan ...." spellcheck="false" required>{{ old('pertanyaan') }}</textarea>
                                @error('pertanyaan')
                            <p class='text-danger inputerror'>{{ $message }} </p>
                            @enderror
                            </div>
